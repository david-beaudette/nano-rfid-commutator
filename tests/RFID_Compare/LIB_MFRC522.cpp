...
boolean MFRC522::digitalSelfTestPass() {
  ...
  byte selfTestResultV0[] =  {0x00, 0x87, 0x98, 0x0f, 0x49, 0xFF, 0x07, 0x19,
  ...
  switch(getFirmwareVersion()) {
    case 0x90 :
      selfTestResult = selfTestResultV0;
      break;
    case 0x91 :
  ...
  byte readFifoByte = 0;
  for (i=0; i < 64; i++) {
    readFifoByte = readFromRegister(FIFODataReg);
    if (readFifoByte != selfTestResult[i]) {
      
      Serial.print("At index ");
      Serial.println(i);
      Serial.print("The expected byte is ");
      Serial.println(selfTestResult[i], HEX);
      Serial.print("The byte read is ");
      Serial.println(readFifoByte, HEX);
      return false;
    }
  }
  return true;
...
int MFRC522::commandTag(byte cmd, byte *data, int dlen, byte *result, int *rlen) {
  ...
  if (i != 0) { // Request did not time out.
    if(!(readFromRegister(ErrorReg) & 0x1B)) {  // BufferOvfl Collerr CRCErr ProtocolErr
      status = MI_OK;
      if (n & irqEn & 0x01) {
        //Serial.println("Reader time out.");
        status = MI_NOTAGERR;
      }
      ...
      Serial.print("Reader returned error code: ");
      Serial.println(readFromRegister(ErrorReg), HEX);
      status = MI_ERR;
    }
  } else {
    Serial.println("Reader did not time out as expected.");
    Serial.print("Timer counter content: ");
    Serial.print(readFromRegister(TCounterValueRegH), HEX);
    Serial.println(readFromRegister(TCounterValueRegL), HEX);
    status = MI_ERR;
  }
  return status;
}
...
/*!
  ...
  @param   mode  The mode we are requesting in.
